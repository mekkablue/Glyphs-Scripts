...
class VerticalMetricsManager(object):
	...
	def __init__(self):
		...
		self.w.useTypoMetrics = vanilla.CheckBox(
			(inset + 70, linePos, -inset, 20), u"Use Typo Metrics (fsSelection bit 7)", value=True, callback=self.SavePreferences, sizeStyle='small'
			)
		...
		self.w.includeAllMasters = vanilla.CheckBox(
			(inset, linePos, -inset, 20), u"Include all masters (otherwise current master only)", value=True, callback=self.SavePreferences, sizeStyle='small'
			)
		...
		self.w.allOpenFonts = vanilla.CheckBox(
			(inset, linePos - 1, -inset, 20), u"⚠️ Read out and apply to ALL open fonts", value=False, callback=self.SavePreferences, sizeStyle='small'
			)
		self.w.allOpenFonts.getNSButton().setToolTip_(
			u"If activated, does not only measure the frontmost font, but all open fonts. Careful: when you press the Apply button, will also apply it to all open fonts. Useful if you have all font files for a font family open."
			)
		linePos += lineHeight
	...
	def updateUI(self, sender=None):
		...
		self.w.runButton.setTitle("Apply to Font%s" % ("s" if self.w.allOpenFonts.get() else ""))
