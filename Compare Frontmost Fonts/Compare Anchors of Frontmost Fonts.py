...
class CompareAnchorsOfFrontmostFonts(object):
	...
	def __init__(self):
		...
		self.w.descriptionText = vanilla.TextBox(
			...
			"Lists all differences in anchor structures and positions in a detailed report in the Macro Window.",
			sizeStyle='small',
			selectable=True
			)
		...
		self.w.reportAnchorHeights = vanilla.CheckBox(
			(inset, linePos - 1, -inset, 20), "Also report differences in anchor heights", value=True, callback=self.SavePreferences, sizeStyle='small'
			)
		self.w.reportAnchorHeights.getNSButton(
		).setToolTip_("Lists anchors in corresponding glyphs if their y coordinates differ more than the threshold set below. Otherwise, only report anchor structures.")
		linePos += lineHeight
		...
		self.w.anchorHeightTolerance.getNSTextField().setToolTip_(
			"Will not report if the difference in y coordinates is less or same as the given tolerance. Use this if e.g. your x-height is different in your italic. Set to zero or leave blank for exact match."
			)
		linePos += lineHeight
		...
		self.w.ignoreHashtaggedAnchors = vanilla.CheckBox(
			(inset, linePos - 1, -inset, 20), "Ignore #hashtagged anchors", value=False, callback=self.SavePreferences, sizeStyle='small'
			)
		...
		self.w.reportOnlyTopBottomCenter = vanilla.CheckBox(
			(inset, linePos - 1, -inset, 20), "Report only top, bottom, center and corresponding anchors", value=False, callback=self.SavePreferences, sizeStyle='small'
			)
		...
		).setToolTip_("Also report if glyph is set to not export. Recommended because non-exporting glyphs may appear as components in other glyphs.")
		linePos += lineHeight

		self.w.openTabAndSelectAnchors = vanilla.CheckBox(
			(inset, linePos - 1, -inset, 20), "Open tabs with affected glyph layers and preselect anchors", value=False, callback=self.SavePreferences, sizeStyle='small'
			)
