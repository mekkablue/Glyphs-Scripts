...
class TravelTracker(object):
	...
	def __init__(self):
		...
		self.w.travelPercentage.getNSTextField().setToolTip_(
			"Anything above ~50% is suspicious in a weight interpolation, and above 70% in a width interpolation. (100% is the diagonal of the bounding box of the path (or glyph) the node belongs to.)"
			)
		linePos += lineHeight
	...
	def maxSegmentRotationForLayers(self, layer, otherLayer):
		...
		for pi, p1 in enumerate(layer.paths):
			...
			for ni, n1 in enumerate(p1.nodes):
				if n1.type == OFFCURVE and n1.nextNode.type == OFFCURVE or (checkForShortSegments and thresholdLength >= distance(n1.position, n1.nextNode.position)):
					continue
				...
				if checkForOrthogonalToNonOrthogonal:
					losingOrthogonalAngle = losingOrthogonalAngle or (
						(angle1%90==0 and angle2%90 != 0) or
						(angle1%90!=0 and angle2%90 == 0)
					)
				...
				if (checkForSegmentRotation and rotation > thresholdAngle) or losingOrthogonalAngle:
					for node in (n1, n1.nextNode):
						if not node in layer.selection:
							layer.selection.append(node)
					for node in (n2, n2.nextNode):
						if not node in otherLayer.selection:
		...
		try:
			...
			if totalAffectedGlyphs==0:
				Message(
					...
					message=f"No glyph found where a node travels and rotation exceed the given thresholds. Congratulations!",
					OKButton="🥂 Cheers!",
					)
