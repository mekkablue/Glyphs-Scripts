...
def showAllMastersOfGlyphInCurrentTab(thisGlyphName):
	...
	if thisGlyph:
		...
		thisTab.layers = [layer for layer in thisGlyph.layers if layer.isMasterLayer or layer.isSpecialLayer]
...
def showAllMastersOfGlyphs(glyphNames, openNewTab=True, avoidDuplicates=True):
	if avoidDuplicates:
		glyphNamesSet = []
		[glyphNamesSet.append(g) for g in glyphNames if g not in glyphNamesSet]
