...
class BatchAddSmartAxes(object):
	...
	def __init__( self ):
		...
		if not self.LoadPreferences():
			print("⚠️ ‘Batch-Add Smart Axes’ could not load preferences. Will resort to defaults.")
	...
	def BatchAddSmartAxesMain( self, sender=None ):
		try:
			...
			if thisFont is None:
				...
				axisInput = [
					l.replace(" ","").strip() for l in input.strip().splitlines() 
					if l.split("#")[0].count(":")==2
				]
				...
				for selectedLayer in thisFont.selectedLayers:
					...
					for axisInfo in axisInput:
						...
						if not axisName in existingAxes:
					...
					for axisInfo in axisInput:
						...
						for master in thisFont.masters:
							...
							for anyLayer in [l for l in thisGlyph.layers if l.associatedMasterId==mID and not l.isMasterLayer]:
