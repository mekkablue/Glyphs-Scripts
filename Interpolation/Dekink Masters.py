	...
	try:
		if refi1 != None and refi2 != None:
			...
			for thisID in compatibleLayerIDs:
				...
				newPosition = NSPoint( 
					refNodeA.x+refVector.width*ratio,
					refNodeA.y+refVector.height*ratio 
				)
...
layerIDs = []
for subrunArray in currentGlyph.layerGroups_masters_error_( NSArray(currentFont.instances), NSArray(currentFont.masters), None ):
