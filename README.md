...
* **Build Rare Symbols:** Builds white and black, small and large, circles, triangles and squares. Also builds `servicemark` (℠) and `telephone` (℡) from existing `M` and `trademark` glyphs.
...
* **CPAL Manager:** Manage color palette (CPAL) indices in color layers: for specified color indexes, delete, disable, reassign or duplicate color layers, or reverse and rotate colors. 
...
* **Copy Layer to Layer:** Copies paths (and optionally, also components, anchors, hints and metrics) from one layer to another, across selected glyphs, fonts and masters, including backgrounds, color palette layers, and layers that do not yet exist in the target.
...
* **Reproject Axis:** Rescales all design-space values of an axis onto a new min/max range. Recalculates master and instance coordinates, name particle values (Glyphs 4), brace (intermediate) and bracket (alternate) layer coordinates, Virtual Master locations, and the axis values in condition feature code.
