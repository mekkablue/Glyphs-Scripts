...
# -*- coding: utf-8 -*-
__doc__="""
Opens a new tab with glyphs containing zero handles, bad outline orders or path directions.
"""
...
for thisGlyph in thisFont.glyphs:
	...
	for thisLayer in thisGlyph.layers:
		if hasZeroHandles( thisLayer ):
			print "%s: found zero handle on Layer '%s'." % ( glyphName, thisLayer.name )
			zeroHandles.append( glyphName )
		if hasBadOutlineOrder( thisLayer ):
			print "%s: found bad path direction on Layer '%s'." % ( glyphName, thisLayer.name )
