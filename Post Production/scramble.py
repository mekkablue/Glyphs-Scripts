...
def getRvrnGlyphs(font):
	"""Collect all glyphs referenced in the rvrn feature's lookups."""
	glyphs = set()
	if 'GSUB' not in font:
		return glyphs
	...
	rvrnLookupIndices = set()
	for featRecord in gsub.FeatureList.FeatureRecord:
		if featRecord.FeatureTag == 'rvrn':
			for idx in featRecord.Feature.LookupListIndex:
				rvrnLookupIndices.add(idx)

	for idx in rvrnLookupIndices:
		lookup = gsub.LookupList.Lookup[idx]
		for subtable in lookup.SubTable:
			if hasattr(subtable, 'mapping'):
				glyphs.update(subtable.mapping.keys())
				glyphs.update(subtable.mapping.values())
			elif hasattr(subtable, 'ligatures'):
				for glyph, ligList in subtable.ligatures.items():
					glyphs.add(glyph)
					for lig in ligList:
						glyphs.add(lig.LigGlyph)
						glyphs.update(lig.Component)
			elif hasattr(subtable, 'substitutes'):
				for glyph in subtable.substitutes:
					glyphs.add(glyph)

	if glyphs:
		print(f'[DEBUG] rvrn glyphs excluded from scrambling: {len(glyphs)}')
	return glyphs
	...
	try:
		...
		if not gsub.ScriptList or not gsub.ScriptList.ScriptRecord:
			...
			for scriptRec in gsub.ScriptList.ScriptRecord:
				...
				if script.DefaultLangSys:
					if 0 not in script.DefaultLangSys.FeatureIndex:
						script.DefaultLangSys.FeatureIndex = [0] + list(script.DefaultLangSys.FeatureIndex)
				...
				if script.LangSysRecord:
					for langSysRec in script.LangSysRecord:
						langSys = langSysRec.LangSys
						if 0 not in langSys.FeatureIndex:
							langSys.FeatureIndex = [0] + list(langSys.FeatureIndex)
