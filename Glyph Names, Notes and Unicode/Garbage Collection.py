...
class GarbageCollection( object ):
	...
	def __init__( self ):
		...
		self.w.descriptionText = vanilla.TextBox( (inset, linePos+2, -inset, lineHeight*1.8), "Removes the following items from the glyphs in the frontmost font:", sizeStyle='small', selectable=True )
				...
				if self.pref("userDataFont"):
					...
					if thisFont.userData:
						...
						if len(keysToRemove)>0:
							self.log("\t🚫 %i font.userData entr%s%s%s" %(
								len(keysToRemove),
				...
				if self.pref("userDataMasters"):
					for thisMaster in thisFont.masters:
						...
						if thisMaster.userData:
							...
							if len(keysToRemove)>0:
								self.log("\t🚫 %i master.userData entr%s: %s" %(
									len(keysToRemove),
									"y" if len(keysToRemove)==1 else "ies",
									", ".join(keysToRemove) if keysToRemove else "",
								))
				...
				if self.pref("userDataInstances"):
					for thisInstance in thisFont.instances:
						...
						if thisInstance.userData:
							...
							if len(keysToRemove)>0:
								self.log("\t🚫 %i instance.userData entr%s: %s" %(
									len(keysToRemove),
									"y" if len(keysToRemove)==1 else "ies",
									", ".join(keysToRemove) if keysToRemove else "",
								))
