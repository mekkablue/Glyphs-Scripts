...
class RemoveNonStandardAnchors(object):
	...
	def __init__(self):
		...
		self.w.text_1 = vanilla.TextBox(
			(inset, currentHeight + 2, -inset, 28), "Removes anchors from selected glyphs, that should not be there by default, like ogonek from J.", sizeStyle='small'
			)
		currentHeight += 35
		self.w.keepExtensions = vanilla.CheckBox(
			(inset, currentHeight, -inset, 20), "Keep underscore variants of standard anchors (e.g. top_low)", value=False, callback=self.SavePreferences, sizeStyle='small'
			)
		currentHeight += 20
		self.w.keepExitAndEntry = vanilla.CheckBox(
			(inset, currentHeight, -inset, 20), "Keep exit and entry anchors", value=False, callback=self.SavePreferences, sizeStyle='small'
			)
		currentHeight += 20
		self.w.keepCarets = vanilla.CheckBox(
			(inset, currentHeight, -inset, 20), "Keep caret anchors (only in ligatures)", value=True, callback=self.SavePreferences, sizeStyle='small'
			)
		currentHeight += 20
		self.w.keepNoDefaults = vanilla.CheckBox(
			(inset, currentHeight, -inset, 20), "Ignore glyphs that have no default anchors", value=False, callback=self.SavePreferences, sizeStyle='small'
			)
