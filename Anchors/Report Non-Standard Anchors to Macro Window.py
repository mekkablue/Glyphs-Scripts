...
class ReportNonStandardAnchorsInMacroWindow( object ):
	def __init__( self ):
		...
		# UI elements:
		inset = 10
		currentHeight = 8; self.w.text_1 = vanilla.TextBox( (inset, currentHeight+2, -inset, 28), __doc__.strip(), sizeStyle='small' )
		currentHeight += 35; self.w.includeNonExporting = vanilla.CheckBox( (inset, currentHeight, -inset, 20), "Include non-exporting glyphs", value=False, callback=self.SavePreferences, sizeStyle='small' )
		currentHeight += 20; self.w.reportExitEntry = vanilla.CheckBox( (inset, currentHeight, -inset, 20), "Report exit and entry anchors", value=False, callback=self.SavePreferences, sizeStyle='small' )
		currentHeight += 20; self.w.reportCarets = vanilla.CheckBox( (inset, currentHeight, -inset, 20), "Report ligature caret anchors", value=False, callback=self.SavePreferences, sizeStyle='small' )
