...
if Glyphs.versionNumber >= 3:
	...
	lowercaseMissing = [n for n in lowercase if f"{n}.sc" not in smallcaps or f"{n}.smcp" in smallcaps]
	...
	uppercaseMissing = [n for n in uppercase if f"{n}.sc" not in smallcaps or f"{n}.c2sc" in smallcaps]
	...
	def smallcapVsLowercase(smallCap):
		...
		if isALetter(smallCapName):
			...
			if posOfFirstDot != -1:
				...
				if not lc or lc not in listOfLowercase:
	...
	def capVsSmallcap(cap):
		...
		if not smallCap or smallCap not in listOfSmallCaps:
