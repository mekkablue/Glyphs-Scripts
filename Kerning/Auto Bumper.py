...
class Bumper(object):
	...
	def __init__(self):
		...
		self.w.leftGlyphs.getNSComboBox().setToolTip_(
			"Measures the specified glyphs from their right side to the following glyph. You can type the character ‘é’ or the slash-escaped glyph name ‘/eacute’. Or specify a category after an at sign ‘@Letter’, add a subcategory after a colon ‘@Letter:Lowercase’.\nAdd default strings in the text box at the bottom of the window. Expand the window at the bottom to access it."
			)
		...
		self.w.leftIsGroups.getNSButton().setToolTip_(
			"If on, will measure only the specified glyph, but set the calculated kerning for the whole right group of the glyph (i.e., add group kerning). If off, will set the kerning for the glyph only (i.e., add an exception)."
			)
		linePos += lineHeight
		...
		self.w.rightGlyphs.getNSComboBox().setToolTip_(
			"Measures from the previous glyphs to the specified glyphs to their left side. You can type the character ‘é’ or the slash-escaped glyph name ‘/eacute’. Or specify a category after an at sign ‘@Letter’, add a subcategory after a colon ‘@Letter:Lowercase’.\nAdd default strings in the text box at the bottom of the window. Expand the window at the bottom to access it."
			)
		...
		self.w.rightIsGroups.getNSButton().setToolTip_(
			"If on, will measure only the specified glyph, but set the calculated kerning for the whole left group of the glyph (i.e., add group kerning). If off, will set the kerning for the glyph only (i.e., add an exception)."
			)
		linePos += lineHeight
		...
		self.w.speedPopup = vanilla.PopUpButton(
			(inset + 42, linePos + 1, 80, 17), ("very slow", "slow", "medium", "fast", "very fast"), callback=self.SavePreferences, sizeStyle='small'
			)
		self.w.speedPopup.getNSPopUpButton(
		).setToolTip_("Specifies the number of measurements. Measuring is processor-intensive and can take a while. Slow: many measurements, fast: few measurements.")
		...
		self.w.keepExistingKerning = vanilla.CheckBox(
			(inset + 5, linePos, -inset, 17), "Keep (don’t overwrite) existing kerning", value=True, sizeStyle='small', callback=self.SavePreferences
			)
		...
		self.w.reportInMacroWindow = vanilla.CheckBox(
			(inset + 5, linePos, -inset, 17), "Also report in Macro Window (a few seconds slower)", value=False, sizeStyle='small', callback=self.SavePreferences
			)
		...
		self.w.openNewTabWithKernPairs = vanilla.CheckBox(
			(inset + 5, linePos, 200, 17), "Open Edit tab with new kern pairs", value=False, sizeStyle='small', callback=self.SavePreferences
			)
		...
		self.w.reuseCurrentTab.getNSButton().setToolTip_(
			"If enabled, will not open a new tab with newly added kern pairs, but reuse the current Edit tab. Will open an Edit tab if none is open. Only available in connection with the Open Edit Tab checkbox."
			)
		linePos += lineHeight
